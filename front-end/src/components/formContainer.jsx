...
const FormContainer = ({ title, inputs, actionText, actionLink,additionalText,includeCheckbox  }) => {
  return (
      ...
      <section className="flex w-[25rem] shadow rounded p-8 flex-col gap-14 border bg-white">
        ...
        {inputs.map((input, index) => (
          <div key={index} className="w-full transform border-b-2 bg-transparent text-lg duration-300 focus-within:border-gray-500">
            <input type={input.type} placeholder={input.placeholder} className="w-full border-none bg-transparent outline-none focus:outline-none" />
          </div>
        ))}
        {includeCheckbox && (
        ...
        <Link to={actionLink} className="transform rounded bg-gray-600 text-xl py-3 font-bold duration-300 hover:bg-gray-500 text-white text-opacity-50">
          {actionText}
        </Link>
      </section>
